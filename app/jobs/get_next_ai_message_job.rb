class GetNextAIMessageJob < ApplicationJob
  ...
  def set_ollama_error
    @message.content_text = "(You need to enter a valid API key for Ollama. Click your Profile in the bottom " +
      "left and then Settings. You will find Ollama Key instructions.)"
  end
  ...
  def set_unexpected_error(msg)
    @message.content_text = "(Received a unexpected response from the API after retrying 3 times. The AI servers may be experiencing trouble. " +
      ...
      "#{msg}\n\nIt's also helpful if you report this to the app developers at: https://github.com/allyourbot/bsmai/discussions)"
  end
  ...
  def set_billing_error
    ...
    @message.content_text = "(Received a quota error. Your API key is probably valid but you may need to adding billing details. You are using " +
      "#{service} so please add a credit card, or if you already have one review your billing plan.)"
  end
